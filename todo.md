...
- [x] 7c. `printing.qmd` — short prose around existing records (`Rprintf`/`REprintf`, `Rf_PrintValue`, `R_ShowMessage`, `R_FlushConsole`); WRE §6.5 backlink
- [x] 7d. `serialisation.qmd` — prose polish; custom pstreams (§6.16), XDR helpers
- [x] 7e. `rng.qmd` — new records + prose: `GetRNGstate`/`PutRNGstate`, `unif_rand` family, `R_unif_index`; relationship to `.Random.seed`
- [x] 7f. `math.qmd` — new records + prose: distribution functions (d/p/q/r table), mathematical functions (bessel, gamma, beta...), numerical utilities (`R_pow`, `fmax2`, `imin2`, ...), constants (`M_PI` etc.), standalone Rmath (§6.20 folded in)
- [x] 7g. `numerical.qmd` — new records + prose: optimization (§6.8), integration (§6.9: `Rdqagi`, `Rdqags`, `R_init_stats`), linear algebra / BLAS / LAPACK headers and `FCONE` (§6.11), `findInterval`, `R_max_col`
- [x] 7h. `utilities.qmd` — reorganise into sections + prose: sorting/ordering, matching, `R_compute_identical`, duplicated, options, numeric parsing, paths/tempfiles, platform info (§6.17)
- [x] 7h2. Audit every remaining record in `functions/utilities.yaml` and consider whether it belongs elsewhere (candidates: `Rf_setVar`, `Rf_lcons`, `Rf_substitute`, `Rf_elt`, `Rf_conformable`, `Rf_isTs`, `Rf_namesgets`, `Rf_isUserBinop`, `Rf_stringPositionTr`, `IndexWidth`, `R_popen`); some may be non-API and belong in compliance.yaml
- [x] 7i. Gap records → Part II chapters (modern §6.23 API, each with `since:` when 4.2+): `R_getVar` family + `R_ParentEnv` (4.5.0, environments "Get values"); new experimental "Inspecting bindings" section (environments) with the R 4.6.0 binding/dots API (`R_GetBindingType`, `R_Make*Binding`, `R_*BindingExpression`, `R_envSymbols`, `R_findDotsEnv`, `R_Dots*`, `R_GetDotType`, `R_DotsElt` family); `R_mkClosure` + `R_Closure*` (4.5.0, new functions.qmd "Closures" section); `Rf_allocLang` (4.4.1 per WRE, pairlists); `STRING_PTR_RO` (vectors); `Rf_isDataFrame` (4.5.0, attributes "Data frames" — added the missing chunk); `R_getAttributes` family, `R_nrow`/`R_ncol`, `R_class` (4.6.0, attributes); `R_mapAttrib` (4.6.0, experimental)
- [x] 7j. Gap records → misc: `R_registerRoutines`/`R_useDynamicSymbols` (new `functions/calling-c.yaml`; added missing `_setup.qmd` include to calling-c.qmd); experimental resizable vectors `R_allocResizableVector`/`R_duplicateAsResizable`/`R_resizeVector` (+`R_isResizable`/`R_maxLength`, 4.6.0, new vectors.qmd "Resizable vectors" section). `SET_GROWABLE_BIT` stays non-API (only in the backport recipe)
