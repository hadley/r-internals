...
- [ ] 7h2. Audit every remaining record in `functions/utilities.yaml` and consider whether it belongs elsewhere (candidates: `Rf_setVar`, `Rf_lcons`, `Rf_substitute`, `Rf_elt`, `Rf_conformable`, `Rf_isTs`, `Rf_namesgets`, `Rf_isUserBinop`, `Rf_stringPositionTr`, `IndexWidth`, `R_popen`); some may be non-API and belong in compliance.yaml
